...
// // ✅ Fetch Doctor's Sessions
// const fetchDoctorSessions = asyncHandler(async (req, res) => {
//     try {
//         const doctorId = req.user.id; // Doctor ID from token
//         console.log(`🔍 Fetching sessions for Doctor ID: ${doctorId}`);
...
//     const doctorId = req.user.id; // Assuming `req.user.id` is set after authentication

//     const sessions = await Session.find({ doctor: doctorId })
//         .populate('patient', 'name email') 
//         .populate('service', 'name')
//         .sort({ date: -1 }); // Sort by latest sessions
...
// module.exports = { getCategories, getServices,viewUpcomingSessions,viewPatientProfile,enrollPricing ,updateAvailability ,createSessionUnderDoctor,viewUserProfile, fetchDoctorSessions,getAllDoctorSessions};
